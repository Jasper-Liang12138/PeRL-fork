...
用法:
    python scripts/download_qwen25_7b_modelscope.py
    python scripts/download_qwen25_7b_modelscope.py --model_id Qwen/Qwen2.5-7B-Instruct --save_dir /path/to/models
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="通过 ModelScope 下载 Qwen2.5-7B 模型"
    )
    parser.add_argument(
        "--model_id",
        type=str,
        default="Qwen/Qwen2.5-7B-Instruct",
        help="ModelScope 模型 ID (默认: Qwen/Qwen2.5-7B-Instruct)",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/mnt/nvme0/models/Qwen2.5-7B-Instruct",
        help="本地保存目录 (默认: ./models/Qwen2.5-7B-Instruct)",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
